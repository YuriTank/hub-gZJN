...
#重点修改函数
#生成lables：["B-address","I-book","B-book","I-address",]  id2label : {1:"B-address",2:"I-book"} , label2id :{"B-address":1,"I-book":2}
def build_label_schema() -> tuple[list[str], dict[str, int], dict[int, str]]:
    """构建 BIO 标签体系，返回 (labels, label2id, id2label)。"""
    labels = []
    with open(DATA_DIR / f"label_names.json", "r",encoding="utf-8") as f:
        labels = json.load(f)
...
def span_to_bio(ner_tags: list, label2id: dict) -> list[int]:
...
class PeopleDailyDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        ner_tags: dict = row.get("ner_tags") or []

        # 1. span → 字符级 BIO id 列表
        char_labels = span_to_bio( ner_tags, self.label2id)
        ...
        #aligned_labels 至少比原始的 char_labels 长度多2， 长度和BERT分词 后的input_ids一致
