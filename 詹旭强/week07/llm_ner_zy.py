...
def build_client() -> OpenAI:
    ...
    api_key = "sk-827b13a71cba42bb8be68baa1d10d359"
    if not api_key:
...
def sample_records(n: int, seed: int = 42) -> list[dict]:
    ...
    sampled_raw = selected_list[:n]

    # 2. 将 BIO 格式转换为原有的 text + label 字典格式
    converted_results = []
    for record in sampled_raw:
        ...
        converted_results.append({"text": text, "label": label})
...
def main():
   ...
   # print(f"采样 {records} ")
    ...
    out_path = LOG_DIR / "eval_llm_zy.json"
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(result, f, ensure_ascii=False, indent=2)
    print(f"\nLLM 评估结果已保存 → {out_path}")
    print("\n下一步：python compare_results_zy.py")
