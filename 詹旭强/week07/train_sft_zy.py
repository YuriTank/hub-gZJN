...
SYSTEM_PROMPT = (
    ...
    "实体类型（英文标识）：LOC（地址）、ORG（公司）、PER（人名）"
...
class SFTDataset(Dataset):
    """
    把 peoles_daily NER 数据转换为 chat-format SFT 训练样本。
...
def main():
    ...
    ckpt_dir   = output_dir / ("sft_full_ckpt_zy" if args.full_ft else "sft_adapter_zy")
    ...
    #转换一下数据结构   
    train_raw = convert_bio_to_label_format(train_raw)
    val_raw = convert_bio_to_label_format(val_raw)
    ...
    model = AutoModelForCausalLM.from_pretrained(
        ...
        dtype=torch.bfloat16,  # transformers 5.x 用 dtype= 不用 torch_dtype=
    ...
    log_path = output_dir / "logs" / f"train_{log_tag}_zy.json"
    ...
    print(f"\n下一步：python evaluate_sft_zy.py 查看 entity F1 与多方对比")
