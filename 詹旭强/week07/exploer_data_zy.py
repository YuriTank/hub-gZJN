...
def collect_stats(records: list) -> dict:
    ...
    for row in records:
        ...
        # 1. 计算文本长度（按 token 数量统计，因为输入是已分词的列表）
        text_lengths.append(len(tokens))
        
        total_entities = 0
        
        # 2. 遍历标签，解析 BIO 序列以提取实体
        i = 0
        while i < len(ner_tags):
            tag = ner_tags[i]
            
            # 当遇到 B- (Begin) 标签时，开始提取实体
            if tag.startswith("B-"):
                etype = tag.split("-", 1)[1]  # 提取实体类型，如 "ORG"
                start_idx = i
                surface_tokens = [tokens[i]]
                i += 1
                
                # 继续向后寻找连续的 I- (Inside) 标签
                while i < len(ner_tags) and ner_tags[i].startswith("I-") and ner_tags[i].split("-", 1)[1] == etype:
                    surface_tokens.append(tokens[i])
                    i += 1
                    
                end_idx = i - 1  # 实体的结束索引
                surface_form = "".join(surface_tokens)  # 拼接成完整的实体词面
                
                # 3. 更新各项统计数据（与原逻辑完全一致）
                entity_type_counts[etype] += 1
                entity_lengths.append(end_idx - start_idx + 1)
                total_entities += 1
                
                if etype not in entities_by_type:
                    entities_by_type[etype] = []
                entities_by_type[etype].append(surface_form)
            else:
                # O 标签或其他情况，直接跳过
                i += 1
...
def plot_entity_distribution(stats_train: dict):
    ...
    fig.savefig(FIG_DIR / "entity_distribution_zy.png", dpi=120)
    print(f"  已保存 → {FIG_DIR / 'entity_distribution_zy.png'}")
    plt.close()
...
def plot_text_length_distribution(stats_train: dict):
    ...
    ax.legend()
    plt.tight_layout()
    fig.savefig(FIG_DIR / "text_length_distribution_zy.png", dpi=120)
    print(f"  已保存 → {FIG_DIR / 'text_length_distribution_zy.png'}")
    plt.close()
...
def plot_entity_length_distribution(stats_train: dict):
    ...
    ax.set_ylabel("出现次数")
    plt.tight_layout()
    fig.savefig(FIG_DIR / "entity_length_distribution_zy.png", dpi=120)
    print(f"  已保存 → {FIG_DIR / 'entity_length_distribution_zy.png'}")
    plt.close()
...
def main():
    ...
    print("下一步：python train_zy.py               # 训练 BERT+Linear")
    print("         python train_zy.py --use_crf    # 训练 BERT+CRF")
