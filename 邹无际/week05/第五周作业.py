...
class LM(nn.Module):
    def __init__(self, vocab_size, embed_dim, hidden_dim, num_layers, model_type, dropout):
        ...
        if model_type == "transformer":
            ...
            self.pos_encoder = nn.Parameter(torch.randn(1, 5000, embed_dim) * 0.01)
    ...
    def forward(self, x):
        if self.model_type == "transformer":
            src = self.embed(x) + self.pos_encoder[:, :x.size(1), :]
            # 创建因果mask（上三角为True表示屏蔽）
...
def run_epoch(model, loader, criterion, optimizer, device, train=True):
    ...
    for i, (x, y) in enumerate(loader):
        ...
        # 每10个batch打印一次进度
        if train and (i + 1) % 10 == 0:
            print(f"\r  训练进度: {i+1}/{num_batches} batches", end='', flush=True)
...
def main():
    ...
    parser.add_argument("--seq_len",    type=int,   default=32)  # Transformer用较短序列
    parser.add_argument("--batch_size", type=int,   default=32)  # 减小batch_size加速
