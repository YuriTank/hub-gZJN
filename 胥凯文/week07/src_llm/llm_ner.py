...
def build_client() -> OpenAI:
    api_key = "sk-8ec5c92459114d72817c6a38c676afdc"
    if not api_key:
...
def gold_spans_from_record(record: dict) -> set[tuple[str, str, int, int]]:
    """从 BIO 格式记录中提取 gold spans，格式：{(text, type, start, end)}。"""
    spans = set()
...
FEW_SHOT_EXAMPLES = [
    ...
    {
        "text": "北京大学教授王小明在北京发表演讲",
        "output": '{"entities": [{"text": "北京大学", "type": "ORG"}, {"text": "王小明", "type": "PER"}, {"text": "北京", "type": "LOC"}]}'
    },
    {
        "text": "华为公司董事长任正非在深圳总部召开会议",
        "output": '{"entities": [{"text": "华为公司", "type": "ORG"}, {"text": "任正非", "type": "PER"}, {"text": "深圳", "type": "LOC"}]}'
    },
]
...
def main():
    ...
    for i, record in enumerate(records, 1):
        text = "".join(record.get("tokens", []))  # 从 tokens 拼接文本
