...
def collect_stats(records: list) -> dict:
    ...
    for row in records:
        ...
        while i < len(ner_tags):
            ...
            if tag.startswith("B-"):
                # 找到一个实体的开始
                etype = tag[2:]  # 提取实体类型（PER/ORG/LOC）
                entity_len = 1
                ...
                entities_by_type[etype].append("".join(entity_tokens))
            else:
                i += 1
...
def plot_text_length_distribution(stats_train: dict):
    ...
    ax.axvline(x=128, color="red", linestyle="--", linewidth=1.5, label="max_length=128")
    ax.axvline(x=256, color="orange", linestyle="--", linewidth=1.5, label="max_length=256")
