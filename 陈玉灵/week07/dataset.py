...
def build_label_schema(data_dir: Optional[Path] = None) -> tuple[list[str], dict[str, int], dict[int, str]]:
    ...
    若 data_dir 下存在 label_names.json，则使用该文件定义的标签体系；
    否则使用默认 CLUENER 实体类型。
    """
...
class CluenerDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        if "text" in row:
            ...
            tokens = row["tokens"]
            tags = row["ner_tags"]
            char_labels = [self.label2id.get(tag, 0) for tag in tags]
            chars = tokens
        else:
            raise ValueError(
                "Unsupported record format. Expect 'text'/'label' or 'tokens'/'ner_tags'."
            )
