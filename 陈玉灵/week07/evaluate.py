...
BERT_PATH = ROOT.parent.parent.parent / "pretrain_models" / "bert-base-chinese"
DATA_ROOT = ROOT / "data"
CKPT_DIR = ROOT / "outputs_cyl" / "checkpoints"
LOG_DIR = ROOT / "outputs_cyl" / "logs"
...
def main():
    ...
    dataset_tag = args.data_dir.name
    run_tag = f"{dataset_tag}_{'crf' if args.use_crf else 'linear'}"
    ...
    labels, label2id, id2label = build_label_schema(args.data_dir)
...
def parse_args():
    ...
    parser.add_argument("--data_dir", type=Path, default=DATA_ROOT / "peoples_daily",
                        help="评估数据目录，默认使用 data/peoples_daily")
