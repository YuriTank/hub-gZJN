...
输出：
  outputs_cyl/figures/entity_distribution.png   各类实体频次分布
  outputs_cyl/figures/text_length_distribution.png  文本长度分布
  outputs_cyl/figures/entity_length_distribution.png 实体长度分布
"""
...
DATA_ROOT = ROOT / "data"
FIG_DIR = ROOT / "outputs_cyl" / "figures"
...
def collect_stats(records: list) -> dict:
    ...
    def add_entity(etype: str, surface: str, length: int):
    ...
    for row in records:
        if "text" in row:
            ...
            for etype, spans in label.items():
                for surface, positions in spans.items():
                    for start, end in positions:
                        add_entity(etype, surface, end - start + 1)
                        total_entities += 1
        elif "tokens" in row and "ner_tags" in row:
            tokens = row["tokens"]
            tags = row["ner_tags"]
            ...
            for idx, tag in enumerate(tags):
                if tag == "O":
                    if current_entity is not None:
                        etype, surface, start = current_entity
                        add_entity(etype, surface, len(surface))
                        total_entities += 1
                        current_entity = None
                    ...
                    if current_entity is not None:
                        ...
                        total_entities += 1
                    etype = tag[2:]
                    current_entity = (etype, tokens[idx], idx)
                elif tag.startswith("I-"):
                    etype = tag[2:]
                    if current_entity is not None and current_entity[0] == etype:
                        current_entity = (etype, current_entity[1] + tokens[idx], current_entity[2])
                    else:
                        current_entity = None
            if current_entity is not None:
                etype, surface, start = current_entity
            ...
            raise ValueError("Unsupported record format in explore_data. Expect 'text'/'label' or 'tokens'/'ner_tags'.")
...
def main():
    ...
    train_records = load_split("train", args.data_dir)
    val_records = load_split("validation", args.data_dir)

    stats_train = collect_stats(train_records)
    stats_val = collect_stats(val_records)

    print_summary(stats_train, stats_val, args.data_dir.name)
    ...
    print("\n探索完成！图表已保存到 outputs_cyl/figures/")
    print(f"数据目录：{args.data_dir}")
    print("下一步：python train.py --data_dir data/peoples_daily               # 训练 BERT+Linear")
    print("         python train.py --use_crf --data_dir data/peoples_daily    # 训练 BERT+CRF")


def parse_args():
    parser = argparse.ArgumentParser(description="探索 NER 数据集")
    parser.add_argument("--data_dir", type=Path, default=DATA_ROOT / "peoples_daily", 
                        help="探索数据目录，默认使用 data/peoples_daily")
